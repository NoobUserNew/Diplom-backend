...
app.post('/enterprises', (req, res) => {
  ...
  console.log('POST /enterprises received:', { name, image_url, description, slug });
  if (!name || !image_url || !slug) {
    return res.status(400).json({ error: 'Missing required fields: name, image_url, and slug are required' });
  }
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('INSERT INTO enterprises (name, image_url, description, slug) VALUES (?, ?, ?, ?)');
...
app.put('/enterprises/:id', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('UPDATE enterprises SET name = ?, image_url = ?, description = ?, slug = ? WHERE id = ?');
...
app.post('/products', (req, res) => {
  const { name, image_url, description, slug, enterprise_id } = req.body;
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('INSERT INTO products (name, image_url, description, slug, enterprise_id) VALUES (?, ?, ?, ?, ?)');
      const info = stmt.run(name, image_url, description, slug, enterprise_id);
      return { id: info.lastInsertRowid };
    });
    const result = transaction();
    res.status(201).json(result);
...
app.put('/products/:id', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('UPDATE products SET name = ?, image_url = ?, description = ?, slug = ?, enterprise_id = ? WHERE id = ?');
      const info = stmt.run(name, image_url, description, slug, enterprise_id, req.params.id);
      return { updated: info.changes };
    });
    const result = transaction();
    res.json(result);
...
app.post('/news', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('INSERT INTO news (title, image_url, short_description, full_text, slug) VALUES (?, ?, ?, ?, ?)');
      const info = stmt.run(title, image_url, short_description, full_text, slug);
      return { id: info.lastInsertRowid };
    });
    const result = transaction();
    res.status(201).json(result);
...
app.put('/news/:id', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('UPDATE news SET title = ?, image_url = ?, short_description = ?, full_text = ?, slug = ? WHERE id = ?');
      const info = stmt.run(title, image_url, short_description, full_text, slug, req.params.id);
      return { updated: info.changes };
    });
    const result = transaction();
    res.json(result);
  ...
  try {
    ...
    const enrichedSliders = sliders.map(slider => {
      ...
      if (slider.type === 'enterprise') {
        query = db.prepare('SELECT id, name AS title, image_url AS imageUrl, description FROM enterprises WHERE id = ?');
        row = query.get(slider.ref_id);
      } else if (slider.type === 'product') {
        query = db.prepare('SELECT id, name AS title, image_url AS imageUrl, description FROM products WHERE id = ?');
        row = query.get(slider.ref_id);
      } else if (slider.type === 'news') {
        query = db.prepare('SELECT id, title, image_url AS imageUrl, short_description AS description FROM news WHERE id = ?');
        row = query.get(slider.ref_id);
      } else {
        return null;
      }
...
app.get('/sliders/:id', (req, res) => {
  try {
    ...
    if (slider.type === 'enterprise') {
      ...
      query = db.prepare('SELECT id, title, image_url AS imageUrl, short_description AS description, full_text FROM news WHERE id = ?');
      row = query.get(slider.ref_id);
    } else {
...
app.post('/sliders', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('INSERT INTO sliders (type, ref_id, position) VALUES (?, ?, ?)');
...
app.put('/sliders/:id', (req, res) => {
  ...
  try {
    const transaction = withTransaction(() => {
      const stmt = db.prepare('UPDATE sliders SET type = ?, ref_id = ?, position = ? WHERE id = ?');
...
app.post('/login', (req, res) => {
 ...
 if (username === 'admin' && password === 'Admin123') { // Простая проверка
...
app.get('/enterprises', (req, res) => res.json([{ id: 1, name: 'Enterprise 1', image_url: 'http://example.com/1.jpg' }]));
app.get('/products', (req, res) => res.json([{ id: 1, name: 'Product 1', image_url: 'http://example.com/1.jpg' }]));
app.get('/news', (req, res) => res.json([{ id: 1, title: 'News 1', image_url: 'http://example.com/1.jpg' }]));
